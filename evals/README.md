...
|----|-----------|--------|--------------------------------------------|
| `add-skill` | a fresh `zeon new project` tree (or `tests/fixtures/valid_min` copy) | "Add a skill called `tap_plate` that taps the plate twice with the left arm." | `skills/tap_plate/` exists with metadata + code; `tap_plate()` defined; only manifest functions called; no hardcoded gripper widths when an anchor grasp block is available. |
